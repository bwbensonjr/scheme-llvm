...
set is merged into the compile's `macro-env`. On the re-homed doors (the Chez batch driver and
the REPL) the procedures resolve as imported bindings referencing `scheme.base` external
globals and `scheme.base.ll` is linked/loaded; the Chez-free embedded runner
(`scheme-run` / `scheme-compile`) provides the same prelude by prepend pending its follow-on
re-home, so its observable behavior matches. A name the program defines itself SHALL take
precedence over the auto-imported one (user-wins shadowing, per the Stage 0 resolution order).
...
- **THEN** it builds and runs; on the re-homed doors the prelude procedures resolve to
  `(scheme base)` exports and `scheme.base.ll` is linked/loaded into the result

#### Scenario: A derived-form macro works without a prepended prelude

- **WHEN** a program uses `cond`/`case`/`when` without `--no-prelude` on a re-homed door
