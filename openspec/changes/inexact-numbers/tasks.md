...
- [ ] 1.1 In `src/runtime/runtime.c`, add `HDR_FLONUM = 8` to the tag-7 header
...
- [ ] 2.4 Add `val rt_modulo(val a, val b)`: flooring remainder (result sign =
      divisor sign) on the fixnum path; flonum operands via `to_double` + `fmod`
      adjusted to floor; zero divisor → trap.
- [ ] 2.5 Update `rt_eqv_p` (`runtime.c:144`) to compare two flonums by value
...
- [ ] 3.1 Add `rt_flonum_p`, `rt_number_p`, `rt_real_p`, `rt_inexact_p`; refine
      `rt_integer_p` (fixnum ∨ integral flonum) and `rt_exact_p` (fixnum only)
      (`runtime.c:760-761`).
- [ ] 3.2 Add `rt_exact_to_inexact` (fixnum → flonum; flonum unchanged) and
      `rt_inexact_to_exact` (integral flonum → fixnum; non-integral → trap; fixnum
      unchanged).
- [ ] 3.3 Add a `HDR_FLONUM` case to `print_val` (`runtime.c:930`) rendering a
...
- [ ] 3.4 Add `val rt_write_char(val c)` writing the character's UTF-8 bytes to
      stdout (reuse `utf8_encode`, `runtime.c:282`) and returning the unspecified
      value.
- [ ] 3.5 Add a runtime flonum formatter callable from `number->string` (so the
      Scheme side does not reimplement float→string).

## 4. Compiler wiring

- [ ] 4.1 In `src/parse.ss` `*integrable*`, add `(/ %/ 2 …)`, `(modulo %modulo 2)`,
      ...
      to `*prims*` as needed.
- [ ] 4.2 In `src/parse.ss`, accept inexact literals in the bootstrap `const`
...
- [ ] 4.3 In `src/emit.ss`, add prim→runtime rows for `%/ → rt_div`, `%modulo →
...
- [ ] 4.4 In `src/passes/expand.ss`, add `/` to `expand-arith`'s n-ary left fold
      (`(/ a)` → `(/ 1 a)`, `(/)` → error), mirroring `+ - *`.

## 5. Prelude: self-hosted reader, macros, library procs

- [ ] 5.1 In `src/prelude.scm`, extend the `rd-*` reader (`rd-numeric?` and a new
...
- [ ] 5.2 In `src/prelude.scm`, extend `number->string` (~line 248) with a flonum
      branch delegating to the runtime formatter (task 3.5).
- [ ] 5.3 In `src/prelude.scm`, add the `do` `syntax-rules` macro in `(scheme
...
- [ ] 5.4 If any of `modulo`/predicates/conversions is better expressed in the
...
- [ ] 6.1 Regenerate `lib/scheme/base.sld` via `chez --script
      tools/gen-scheme-base.ss` FIRST (so the AOT path sees `do` and the new
      bindings), then verify the staleness guard.
- [ ] 6.2 Run `make regen`; confirm the self-host fixed point converges (try one
...
- [ ] 7.1 Add small demos: `demos/flonum-arith.scm` (float + mixed `+ - * /` and
...
- [ ] 7.2 Register `demos/mandelbrot.scm` and the new small demos in
      `demos/run-tests.sh` with exact expected stdout (capture the mandelbrot ASCII
      art once the feature works), on both backends.

## 8. Verification

- [ ] 8.1 Build `emit run` and run `demos/run-tests.sh` (default `emit-run`
      backend) — all demos pass, including `mandelbrot` and the new ones.
- [ ] 8.2 Run `RUNNER=aot demos/run-tests.sh` — same expected values on the native
      exe; output byte-identical to `emit-run` (including the flonum-printing and
      mandelbrot output).
- [ ] 8.3 Verify the reader-disambiguation and predicate scenarios from the spec:
      `2.5`/`-1.25`/`.5`/`1e3` read as flonums; `2.5.6`/`1e`/`-`/`foo` read as
      symbols; `(= 2 2.0)` `#t` but `(eqv? 2 2.0)` `#f`; `(inexact->exact 2.5)`
      traps.
- [ ] 8.4 Verify float output round-trips: `(= x (read-from-string (number->string
      x)))` for a sample of flonums, on both backends.
- [ ] 8.5 Run `./run-dev-tests.sh` — self-hosting fixed point, backend
...
- [ ] 9.2 Update the README: numeric-tower bullet (fixnums-only → fixnums +
